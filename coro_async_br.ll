; =============================================================================
; LLVM Coroutine Async/Await Demo - Using br instead of switch for WASM workaround
; =============================================================================
...
; ============== Coroutine 1: Two suspend points - Using br instead of switch ==============
...
entry:
  ...
  ; Replace switch with br chain
  %is_resume1 = icmp eq i8 %s1, 0
  br i1 %is_resume1, label %resume1, label %check_cleanup1

check_cleanup1:
  %is_cleanup1 = icmp eq i8 %s1, 1
  br i1 %is_cleanup1, label %cleanup, label %suspend
...
resume1:
  ...
  ; Replace switch with br chain
  %is_resume2 = icmp eq i8 %s2, 0
  br i1 %is_resume2, label %resume2, label %check_cleanup2

check_cleanup2:
  %is_cleanup2 = icmp eq i8 %s2, 1
  br i1 %is_cleanup2, label %cleanup, label %suspend
...
; ============== Coroutine 2: One suspend point - Using br instead of switch ==============
...
entry:
  ...
  ; Replace switch with br chain
  %is_resume = icmp eq i8 %s, 0
  br i1 %is_resume, label %resume, label %check_cleanup

check_cleanup:
  %is_cleanup = icmp eq i8 %s, 1
  br i1 %is_cleanup, label %cleanup, label %suspend
